<?php
namespace Opencart\Catalog\Controller\Extension\NovakurFashion\Event;
...
class NovakurFashion extends \Opencart\System\Engine\Controller {
    ...
    public function view(string &$route, array &$data, string &$code, string &$output): void {
        if ($this->config->get('config_theme') !== 'novakur_fashion') {
            return;
        }

        $template_route = $route;
        if (str_starts_with($template_route, 'extension/novakur_fashion/')) {
            $template_route = substr($template_route, strlen('extension/novakur_fashion/'));
        }

        $template_file = DIR_EXTENSION . 'novakur_fashion/catalog/view/template/' . $template_route . '.twig';
        if (!is_file($template_file)) { return; }

        if ($template_route === 'common/header') {
            $data['nf_primary_color']    = (string)$this->config->get('theme_novakur_fashion_primary_color') ?: '#1a1a1a';
            $data['nf_secondary_color']  = (string)$this->config->get('theme_novakur_fashion_secondary_color') ?: '#c9a96e';
            $data['nf_accent_color']     = (string)$this->config->get('theme_novakur_fashion_accent_color') ?: '#e8d5b7';
            $data['nf_container_width']  = (int)$this->config->get('theme_novakur_fashion_container_width') ?: 1400;
            $data['nf_heading_font']     = (string)$this->config->get('theme_novakur_fashion_heading_font') ?: 'Playfair Display';
            $data['nf_base_font']        = (string)$this->config->get('theme_novakur_fashion_base_font') ?: 'Inter';
            $data['nf_enable_hero']      = (bool)$this->config->get('theme_novakur_fashion_enable_hero_banner');
            $data['nf_enable_lookbook']  = (bool)$this->config->get('theme_novakur_fashion_enable_lookbook');
            $data['nf_enable_featured']  = (bool)$this->config->get('theme_novakur_fashion_enable_featured_products');
            $data['nf_enable_catgrid']   = (bool)$this->config->get('theme_novakur_fashion_enable_category_grid');
            $data['nf_enable_instagram'] = (bool)$this->config->get('theme_novakur_fashion_enable_instagram_grid');
            $data['cart_count'] = $this->cart->countProducts();
            if (!isset($data['menu_items'])) { $data['menu_items'] = []; }
        }
        ...
        if ($template_route === 'checkout/cart') {
            ...
            if ($total)    $data['totals'][] = ['title' => 'Total',     'text' => $this->currency->format($total, $currency)];
        ...
        if ($template_route === 'account/wishlist') {
            ...
            foreach ($this->model_account_wishlist->getWishlist($this->customer->getId()) as $result) {
                ...
                $data['products'][] = [
                    ...
                    'thumb'      => $this->model_tool_image->resize($image, 400, 500),
                    'price'      => $price,
                    'special'    => $special,
        ...
        if (!str_starts_with($route, 'extension/novakur_fashion/')) {
            $route = 'extension/novakur_fashion/' . $route;
        }
    }
    ...
    private function getHomepageProducts(): array {
        ...
        $sql = "SELECT p.product_id, p.image, p.price, p.tax_class_id, pd.name,
                    (SELECT ps.price FROM `" . DB_PREFIX . "product_special` ps WHERE ps.product_id = p.product_id AND (ps.customer_group_id='1' OR ps.customer_group_id='0') AND (ps.date_start='0000-00-00' OR ps.date_start<=NOW()) AND (ps.date_end='0000-00-00' OR ps.date_end>=NOW()) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special
        ...
        foreach ($query->rows as $row) {
            ...
            $products[] = [
                ...
                'thumb'      => $this->model_tool_image->resize($image, 600, 750),
                'name'       => $row['name'],
                'price'      => $price,
                'special'    => $special,
